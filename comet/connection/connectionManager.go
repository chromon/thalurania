...
type ConnectionManager struct {
	// 管理的连接信息
	connections map[uint32]api.IConnection
...
func NewConnectionManager() *ConnectionManager {
	return &ConnectionManager{
		connections: make(map[uint32] api.IConnection),
	}
}
...
// 由 connId 获取连接
func (cm *ConnectionManager) GetConnection(connId uint32) (conn api.IConnection, err error) {
	// 保护共享资源，加读锁
