...
// 接受用户好友请求
func AcceptFriend(m map[string]*flag.Flag, conn net.Conn, opt int32) {
	...
	if err != nil {
		log.Info.Println("serialize server trans pack (accept friend) object err:", err)
		return
	}

	// 封包用户对象消息并发送
	dp := packet.NewDataPack()
	msg, _ := dp.Pack(constants.TCPNetwork, constants.AcceptFriendOption, packet.NewMessage(IdWorker.GetId(), stp))
